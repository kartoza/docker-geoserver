...
# Install GeoNode if argument if true
if [[ ${GEONODE} =~ [Tt][Rr][Uu][Ee] ]];then
  $request https://www.dropbox.com/s/xlich7pmneaupqp/geoserver-${GS_VERSION}.war?dl=1 -O /tmp/geoserver-${GS_VERSION}.war --no-check-certificate && \
  rm -rf /geoserver/webapps/geoserver/* && \
  unzip -o /tmp/geoserver-${GS_VERSION}.war -d /geoserver/webapps/geoserver/ && \
  rm /tmp/geoserver-${GS_VERSION}.war && \
  $request https://www.dropbox.com/s/q0qc2t7d9alo9fk/data-${GS_VERSION}.zip?dl=1 -O ${resources_dir}/data-${GS_VERSION}.zip --no-check-certificate && \
  unzip ${resources_dir}/data-${GS_VERSION}.zip -d /tmp/geonode_data && \
  rm -rf ${CATALINA_HOME}/security && \
  rm -rf ${CATALINA_HOME}/data && \
  cp -r /tmp/geonode_data/data/security ${CATALINA_HOME} && \
  cp -r /tmp/geonode_data/data ${CATALINA_HOME} && \
  rm -rf ${CATALINA_HOME}/data/security
fi

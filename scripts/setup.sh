...
# Install geoserver data directory
if [[ ! -f $resources_dir/data-$GS_VERSION.zip ]]; then
  $request https://build.geo-solutions.it/geonode/geoserver/latest/data-2.18.2.zip -O $resources_dir/data.zip && \
  unzip $resources_dir/data.zip -d $resources_dir && \
  mv $resources_dir/data/* $GEOSERVER_DATA_DIR
fi

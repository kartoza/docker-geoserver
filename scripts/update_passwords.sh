...
if [[ -f ${GEOSERVER_HOME}/start.jar ]]; then
   export GEOSERVER_INSTALL_DIR=${GEOSERVER_HOME}
else
  export GEOSERVER_INSTALL_DIR=${CATALINA_HOME}
fi
...
if [[ -f "${EXTRA_CONFIG_DIR}"/.default_admin_encrypted_pass.txt ]];then
    ...
    export GEOSERVER_ADMIN_DEFAULT_ENCRYPTED_PASSWORD="crypt1:${GEOSERVER_ADMIN_DEFAULT_PASSWORD%?}"
fi

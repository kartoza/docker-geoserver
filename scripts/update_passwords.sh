...
echo
  ...
  if [[ -f "${EXTRA_CONFIG_DIR}"/.default_admin_encrypted_pass.txt ]];then
      ...
      #export GEOSERVER_ADMIN_DEFAULT_PASSWORD=$(grep -o 'password="[^"]*"' ${USERS_XML} | sed 's/password="//;s/"$//')
      #export GEOSERVER_ADMIN_DEFAULT_PASSWORD_DIGEST=$(grep -o 'password="[^"]*"' ${USERS_XML} | sed -E 's/password="(crypt1:|digest1:)?([^"]*)"/\1/')

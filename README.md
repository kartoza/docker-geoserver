...
The recommended approach is to pull the trusted Docker image as follows:
...
1) Navigate to the directory with the docker compose yaml files.
    ```bash
    cd compose
    ```
2) Update `.env` to suit your needs and start the services.

    ```shell
    docker-compose up -d
    ```

**Note** The default login credentials are stored in the `.env` file and 
should be updated to something more secure. 

Once all the services are started, test by visiting the GeoServer landing
...
When reporting issues, especially those related to installed extensions 
(community or stable), please first consult the 
[GeoServer Issue page](https://osgeo-org.atlassian.net/jira/software/c/projects/GEOS/issues)￼ to check whether the issue has already been reported 
upstream. We rely on the GeoServer developers and community to provide support. 
For urgent upstream problems, consider getting paid support from the developers of 
...
by checking upstream reports and testing the issue with newer or older versions.
